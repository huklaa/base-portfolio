(function(){
  ...
  function buildExport(){
    ...
    return {
      ...
      methodology:{activityScore:'local heuristic',economicFingerprint:'local explainable heuristic',behaviorDelta:'latest 30 days compared with previous 30 days',stablecoinFlow:'recent Blockscout ERC-20 transfer records filtered to recognized stablecoin symbols'},
      limits:{transactionHistoryCap:10000,source:'Base Blockscout public APIs',erc8021Detection:'best-effort sentinel detection',stablecoinHistory:'limited to fetched paginated transfer records; not complete lifetime accounting'},
  ...
  function ensureUI(){
    ...
    el.innerHTML=`<div class="card-head"><div><div class="eyebrow">FOR BUILDERS & AGENTS</div><h3>Machine-readable Base profile</h3></div><span class="badge">JSON v1.1</span></div><p class="section-intro">Export the same public analytics in a compact JSON document for experiments, agents, demos, or reproducible analysis. No private wallet data is included.</p><div class="export-actions"><button id="downloadProfileJson" class="primary">Download JSON profile</button><button id="copyProfileUrl" class="secondary-btn">Copy shareable profile URL</button><button id="copyProfileJson" class="secondary-btn">Copy JSON</button></div><pre id="exportPreview" class="export-preview">Analyze a Base address to generate a profile.</pre>`;
